...
module Cosmos
  ...
  module BufferedSocketStream
    ...
    def adopt_channels
      ...
      if @write_socket and adoptable?(@write_socket)
        if @read_socket and @write_socket.equal?(@read_socket) and @read_channel
          ...
          # Not @overflow_policy: that policy is about the READ ring, and this
          # channel's read ring is one nothing will ever drain. Back pressure
          # on it would stall the reader thread against a permanently full ring
          # and leave the bytes piling up in the kernel receive buffer instead.
          # Drop them - they were never going anywhere. Same reasoning, same
          # answer, as the write only serial port in BufferedSerialTransport.
    ...
    #
    # The is_a? gate comes first deliberately, matching what the serial and UDP
    # transports do: probing a double with closed? or remote_address fails the
    # caller's test with an unexpected-message error instead of quietly falling
    # back, and a double that happens to stub those messages would be adopted -
    # handing a made up fileno to the C++ channel. Only something that really
    # is a socket gets probed at all.
    def adoptable?(socket)
      return false unless socket.is_a?(::BasicSocket)
