...
1. **The official client**: `pip install ballcosmos` (verified working
   ...
   **Note**: stock ballcosmos corrupts binary parameters (bytes >= 0x80
   ...
   Both are fixed (along with a connection-reset port leak) on the
   maintained fork — upstream is archived and takes no PRs, so the
   fork's master is the living line (v2.1.0):
   `pip install git+https://github.com/thesamprice/python-ballcosmos`
