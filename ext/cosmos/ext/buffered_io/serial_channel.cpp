...
SerialChannel::SerialChannel(int fd, size_t ring_bytes) : StreamChannel(fd, ring_bytes) {
  ...
  if (::pipe(wake_pipe_) != 0) {
    // Without the self-pipe there is no way to break the reader out of poll(2)
    // on a character device, so stop() could never join it: the channel would
    // be unjoinable for the life of the process. Refuse to build one. The
    // adopter turns this into a clean Ruby exception and falls back to the
    // stock pure Ruby serial path.
    int error = errno;
    wake_pipe_[0] = -1;
    wake_pipe_[1] = -1;
    // The descriptor goes back to the adopter (see StreamChannel's ctor).
    fd_ = -1;
...
bool SerialChannel::transport_send_item(const std::string& item) {
  ...
  while (offset < item.size()) {
    ...
    if (sent == 0) {
      // Nothing written, no error, bytes still to go. Returning true here
      // would report the unsent tail as delivered; latch it instead so the
      // interface disconnects rather than losing part of a command silently.
      latch_errno(EIO);
      return false;
    }
    int error = errno;
