...
public:
  ...
  // (see BufferedChannel#footprint) The ring here grows to fit what is
  // actually buffered rather than being reserved up front, so the honest
  // figure is the live byte count, not the cap.
  virtual size_t footprint() const { return sizeof(*this) + (size_t)buffered_bytes(); }
  ...
  // and copied under mutex_ because the writer thread reads it concurrently.
  ...
  bool writable() const { return socket_connected_ || has_destination(); }
...
protected:
  ...
  // Snapshot the send destination under mutex_. Returns 0 when there is none.
  // Called from the writer thread, which runs outside the lock.
  socklen_t copy_destination(struct sockaddr_storage* out) const;
